...
earnings_client, etf_holdings) staged in S3, parses them into a normalized set
of relationship triples plus Article-node records (filings + scored earnings
releases), writes both locally for inspection, and stages them back to S3
under `processed/` for Phase 2 (graph construction).
...
from src.ingestion.score_sentiment import score_all_transcripts
...
def run() -> tuple[list[dict], list[dict]]:
    ...
    articles = filing_articles + earnings_articles
